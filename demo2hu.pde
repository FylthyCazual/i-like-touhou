...
class Explosion {
  ...
  boolean boom(){
    if(t < 30){
      t++;
      fill(255);
      ellipse((int)pos.x + 25,(int)pos.y + 25, t, t);
     }
    return false;
  }
}
...
class Event {
  ...
  Event(int t,int type, ArrayList<Enemy> spawns){
    this.t = t;
    this.type = type;
    this.spawns = spawns;
  }
...
void runSpellCard (double px, double py, int id, int t, int r) {
  ...
  if (id==0) {
    ...
    spellLength = 5*60;
    if (t%spellLength%5 == 0) {
      Target temp = new Target(0,5);
      spellRot = (int) degrees(temp.angleP1(px,py));
    }
    if (t%spellLength%5 == 0 && t%spellLength < 60) {
      Target temp = new Target(0,5);
      temp.transformTo(r);
...
void draw() {
  ...
  if (mode==0) {
    ...
    if (Events.size()>0 && Events.get(0).t == currentTime) {
      if (Events.get(0).type == 0) {
        for (int i = 0; i < Events.get(0).spawns.size(); i++) { 
          Enemies.add(Events.get(0).spawns.get(i));
        }
      }  
      Events.remove(0);
    }
    if (!paused) {
      ...
      for (int i = 0; i < Enemies.size(); i++) {
        ...
        if (Enemies.get(i).didIDie()) {
          Explosions.add(new Explosion(Enemies.get(i).pos));
  ...
  if (playerType == 0) {
    if (difficulty == 1) {
      if (stage == 0) {
        ...
        temp.add(new Enemy(162.5,0.0,0,1,2));
        Events.add(new Event(600,0,temp));
      }
    }
  }
}